/**
 * Generates an SVG string representing a trash icon.
 * @returns {string} SVG HTML markup string for the trash icon.
 */
function getTrashIconSvg() {
  return `<svg aria-hidden="true" width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
/**
 * Generates an SVG string representing an empty basket graphic.
 ...
 * @param {{id: string, name: string, description: string, price: number, image: string}} product - Product object.
 * @returns {string} Formatted HTML template.
 */
...
/**
 * Generates an anchor link template for category navigation.
 * @param {string} category - Category identifier key.
...
/**
 * Generates HTML markup for a category section header band.
 * @param {string} categoryTitle - Upper-case title of the category.
...
/**
 * Generates HTML wrapper for a product category section grid.
 * @param {string} categoryTitle - Title of the category.
 * @param {string} productsHtml - Concatenated HTML string of product cards.
 * @returns {string} Formatted HTML string.
 */
...
/**
 * Generates HTML markup for displaying an empty basket placeholder.
 * @returns {string} Formatted HTML string.
 */
...
/**
 * Generates HTML markup for a single item entry inside the basket list.
 * @param {{id: string, name: string, price: number, amount: number}} item - Basket item object.
 * @returns {string} Formatted HTML string.
 */
...
/**
 * Generates HTML markup for the order total summary panel.
 * @param {number} subtotal - Subtotal price sum.
 * @param {number} shipping - Delivery fee cost.
...
function getBasketSummaryTemplate(subtotal, shipping, total) {
  return `
        <div class="basket-summary">
            <div class="summary-line"><p>Subtotal</p><p class="subtotal-val">${formatPrice(subtotal)} €</p></div>
...
/**
 * Combines rendered basket item elements and order summary into a filled basket template.
 * @param {string} allBasketItemsHtml - Concatenated HTML string of basket item elements.
 * @param {string} summaryHtml - Formatted HTML string of the basket summary.
 * @returns {string} Formatted combined HTML string.
 */
function getFilledBasketTemplate(allBasketItemsHtml, summaryHtml) {
  return `
        <div id="basket-items-list" class="basket-items-list">
            ${allBasketItemsHtml}
        </div>
        ${summaryHtml}
    `;
